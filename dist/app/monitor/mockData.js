...
exports.generateMockAlerts = exports.generateCustomerHistory = exports.generateProfiles = void 0;
exports.generateTwoYearHistory = generateTwoYearHistory;
const generateProfiles = (count) => {
    ...
    const accountNumbers = Array.from({ length: 20 }, () => "GB" +
    ...
    return Array.from({ length: count }, (_, i) => {
        ...
        const proofOfIdentity = identities[Math.floor(Math.random() * identities.length)];
        const proofOfAddress = addresses[Math.floor(Math.random() * addresses.length)];
        const occupation = occupations[Math.floor(Math.random() * occupations.length)];
        const businessActivity = businessActivities[Math.floor(Math.random() * businessActivities.length)];
        const customerType = customerTypes[Math.floor(Math.random() * customerTypes.length)];
        const sourceOfFunds = sourcesOfFunds[Math.floor(Math.random() * sourcesOfFunds.length)];
        const sourceOfWealth = sourcesOfWealth[Math.floor(Math.random() * sourcesOfWealth.length)];
        const riskRating = riskRatings[Math.floor(Math.random() * riskRatings.length)];
        const riskReason = riskReasons[Math.floor(Math.random() * riskReasons.length)];
        const expectedVolume = volumes[Math.floor(Math.random() * volumes.length)];
        const expectedFrequency = frequencies[Math.floor(Math.random() * frequencies.length)];
        const anticipatedJurisdictions = jurisdictionsList[Math.floor(Math.random() * jurisdictionsList.length)];
        const anticipatedCounterparties = counterpartiesList[Math.floor(Math.random() * counterpartiesList.length)];
        const accountPurpose = purposes[Math.floor(Math.random() * purposes.length)];
        const countryOfResidence = countries[Math.floor(Math.random() * countries.length)];
        const expectedTransactionCountries = transactionCountriesList[Math.floor(Math.random() * transactionCountriesList.length)];
        const dualCitizenship = citizenships[Math.floor(Math.random() * citizenships.length)];
        const linkedJurisdictions = linkedJurisdictionsList[Math.floor(Math.random() * linkedJurisdictionsList.length)];
        const onboardingChannel = channels[Math.floor(Math.random() * channels.length)];
        const type = alertTypes[Math.floor(Math.random() * alertTypes.length)];
        const customer = customers[Math.floor(Math.random() * customers.length)];
        const dateOfBirth = dateOfBirths[Math.floor(Math.random() * customers.length)];
...
const generateCustomerHistory = (count, customerId) => {
...
exports.generateCustomerHistory = generateCustomerHistory;
function generateTwoYearHistory(customerId) {
    const transactions = [];
    const transactionTypes = [
        "Salary Credit",
        "Investment Transfer",
...
const generateMockAlerts = (count = 10000, user, name) => {
    const alertTypes = [
        "Unusual Transaction Size",
        "Rapid Movement of Funds",
        "High-Value Cash Deposits",
    ...
    const descriptions = {
        "Unusual Transaction Size": "Transaction amount significantly deviates from customer's established pattern and risk profile.",
        "Rapid Movement of Funds": "Multiple credits followed by immediate debits suggesting potential fund movement scheme.",
        "High-Value Cash Deposits": "Large cash deposits inconsistent with customer profile and declared income source.",
        Structuring: "Multiple transactions designed to avoid regulatory reporting thresholds under MLR 2017.",
        Smurfing: "Coordinated deposit activity across multiple individuals suggesting organized layering scheme.",
        "Round Amount Transactions": "Frequent exact round-number transactions inconsistent with genuine business activity.",
        "Multiple Small Credits": "Pattern of small credits followed by large debits suggesting fund aggregation activity.",
        "High Risk Country": "Transaction involves jurisdiction classified as high-risk per FATF or OFSI guidance.",
        "Cross-Border Transfers": "Frequent international transfers to secrecy jurisdictions raising capital flight concerns.",
        "High-Risk IP Location": "Transaction initiated from IP address flagged as high-risk or inconsistent with profile.",
        "Dormant Account Reactivation": "Previously inactive account suddenly showing high-value transaction activity.",
        "New Account High Activity": "Recently opened account showing transaction patterns inconsistent with stated purpose.",
        "Behavior Deviation": "Customer activity significantly deviates from established risk profile and KYC data.",
        "Multiple Channels Usage": "Simultaneous usage of multiple banking channels suggesting potential compromise.",
        "Third-Party Payment Systems": "Transactions through unregulated fintech or e-wallet systems raising oversight concerns.",
        "Unusual Product Use": "Account or product usage outside intended purpose per terms and conditions.",
        "Threshold Breach": "Transaction exceeds regulatory reporting thresholds requiring CTR or SAR consideration.",
        "Failed Login Attempts": "Multiple failed authentication attempts suggesting potential account compromise.",
        "PEP Match": "Customer profile matches against politically exposed persons database requiring enhanced due diligence.",
        "Sanctions Hit": "Transaction involves entity or individual on current OFAC, OFSI, or UN sanctions lists.",
        "Adverse Media Match": "Customer associated with negative media coverage raising reputational and compliance risks.",
        "Back-to-Back Transactions": "Mirror transactions with minimal time gaps suggesting potential layering scheme.",
        "Shell Company Transfer": "Transaction involves entity lacking business substance or beneficial ownership transparency.",
        "Crypto Conversion": "Cryptocurrency transactions involving privacy coins, mixers, or anonymous wallets.",
    };
    return Array.from({ length: count }, (_, i) => {
        const type = alertTypes[Math.floor(Math.random() * alertTypes.length)];
        ...
        const location = locations[Math.floor(Math.random() * locations.length)];
        const accountType = accountTypes[Math.floor(Math.random() * accountTypes.length)];
        const monitoringType = monitoringTypes[Math.floor(Math.random() * monitoringTypes.length)];
        let amount;
        if (["High-Value Cash Deposits", "Threshold Breach"].includes(type)) {
            amount = Math.floor(Math.random() * 500000) + 10000;
        }
